...
# 2. 编辑覆盖 sota_snapshot.md（基于 Step 3 的候选 diff，应用到模板产出完整新版本）
```

**5.3 research_updates 追加（若用户 yes）**

- **非 Bootstrap**：用编辑工具在 `<!-- meta-review skill 在此行下方追加新条目，保持最新在上 -->` 注释**正下方**插入新条目（保持最新在上）
- **Bootstrap 模式**：用编辑工具**整段替换** `## [待首次 meta-review 运行] Initial State` ... 末尾占位段，替换为 Update #1 的完整内容
