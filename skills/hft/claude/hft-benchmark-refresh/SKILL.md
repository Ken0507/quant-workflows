...
metadata:
  short-description: "把累积 FA 合并跑 LGBM 选 Top N，落 benchmark{YYYYMMDD}/ 作为新一代 SOTA"
  argument-hint: "[date] [datasets|default] [top_n=100] (可选)"
---
...
**用 `run_in_background=True` 启动 Bash**，然后立即返回向用户报告 PID。
...
6. **不动 lgj 的目录**：`/home/userlgj/` 路径下不读不写（与 HFTPool/CLAUDE.md 实盘红线一致）
