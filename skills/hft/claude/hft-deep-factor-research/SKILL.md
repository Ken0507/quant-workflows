...
metadata:
  short-description: "深度因子研究：OB 观察 → 因子设计 → 主口径验证"
  argument-hint: "[种子想法或FA簇编号]"
---
...
**立即调用 Skill `/ob_mode`**，激活 HFT AI Playground 模式和 ob-investigator MCP 工具。确认进入 OB 模式后再继续后续步骤。
...
- **不要重复提交表达式基本一样的因子**
- **⛔ 禁止修改或续写已有研究**：即使发现高度相关的历史研究，也必须在新的编号目录下开始全新研究。不得修改、追加、或以任何方式改动 `ob_research/` 下已有目录中的任何文件。已有研究仅作为参考阅读
...
```markdown
> **研究执行者**: Claude Code
> **模型**: <例如 Opus 4.7 (1M context)>
> **启动时间**: YYYY-MM-DD HH:MM (UTC+8)
```

要求：
- 如实填写当前运行的模型标识（可参考环境提示或 `/model`），不要省略 `(1M context)` 之类的重要变体信息
- 如果研究过程中模型发生变更（例如用户切换 `/model`），追加一行 `> **期间变更**: <时间> 切换到 <新模型>，原因 <...>`，保留原始启动信息不覆盖
- 如果无法准确辨认模型，写 `Claude (unknown build)` 并在 `research_log.md` 中同步标记待人工确认
...
3. 产出 `research_report.md` 的 §0 部分：核心假设、直觉来源、微观行为猜测、与已有因子关系（执行 §0.3b）
4. **不需要预定义因子列表**
...
>
> **⚠️ 假设修正是研究质量的标志，不是失败。** 最好的研究会在 Phase 1 结束时对现象的理解与 Phase 0 种子假设显著不同。典型的高质量研究路径包括：原始假设被 OB 证据推翻 → 从反例中发现更精确的机制 → 基于修正后的机制设计出更好的因子。如果你的核心假设从 Phase 0 到 Phase 1 结束始终未被挑战或修正，这可能说明探索不够深入。
...
for round in 1..N:
    ...
    3. 按 workflow §1.2 格式完整记录（详细描述现象 + 假设对比分析 + 因子启示 + 新问题）
       - 每轮记录必须包含：观察到的具体现象（带数据）、与预期的对比分析、
         至少 1 个新问题或新假设（驱动下一轮调查的具体方向）
       - 记录因子启示时，除了直接编码方式，也思考该现象在时序维度上的可能性（如信号随时间的演化模式、不同时间尺度上的表现差异），为 Phase 2 的因子设计积累素材
...
```
Agent(prompt="[审查 brief 内容]... 请独立评估：(a) 是否有未被探索的重要 OB 方向？(b) 现有机制理解是否有逻辑漏洞？(c) 因子设计空间是否被充分探索？如果你认为有值得追加的探索方向，请给出具体的、可执行的建议（标的/时段/观察重点）。")
```
...
- **如果有被采纳的建议**：执行对应的 OB 调查（正常走 Reviewer 审查流程）
- **执行完毕后，主进程必须独立思考**：基于新的 OB 发现，是否有自己想深挖的新方向？此时不是"Codex 的建议做完就结束"，而是把 Codex 的建议当作新一轮探索的起点。
  - 如果有新方向 → 继续自主研究（新的 OB 轮次，正常走 Reviewer），直到再次认为饱和 → 回到步骤 1
...
1. **因子设计深度思考（创造性环节，不可跳过）**：
   ...
   - **时序探索（重要的因子衍生维度）**：
     微观信号的原始形态往往不是最优的因子编码。对每个发现都值得思考：通过时序变换能否产出更好的、或额外的因子？时序操作的空间非常广阔，鼓励创造性探索。特别是当一个信号在短 horizon 有预测性时，不要轻易放弃——历史上 #5 spread_dynamics 成功将 tick 级信号通过时序变换桥接到 bar 级，而 #7 spread_state_machine 因为没有尝试时序变换就直接放弃了同类信号。即使信号已经在主 horizon 上有效，时序变换也常常能衍生出捕捉不同信息维度的新因子
...
8. **⚠️ 因子数量检查（Phase 2 → Phase 3 门控）**：
   - 统计通过大样本泛化的因子数量
   - **≥10 个** → 正常进入 Phase 3
   - **5-9 个** → 在 research_log 中记录不足原因，回退到 Phase 1 补充 OB 调查，寻找新的微观机制或从已有发现中衍生更多因子编码方式，然后重新执行 Phase 2 的增量部分（仅处理新增因子）
