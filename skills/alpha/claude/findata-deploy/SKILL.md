...
通过 `~/alpha_projects/.claude/skills/findata-deploy/deploy_to_xinjiang.sh` 把 findata + cube 部署到
...
| 模式 | 命令 | 影响范围 |
|---|---|---|
| **dev**（默认） | `bash ~/alpha_projects/.claude/skills/findata-deploy/deploy_to_xinjiang.sh [--no-test]` | 只更新服务器源码副本 `/data/hftprop/infra/{findata,cube}`（仅 cken 可见，= staging），供自己调试；公共环境与使用方无感知 |
| **release** | `bash ~/alpha_projects/.claude/skills/findata-deploy/deploy_to_xinjiang.sh --release [--no-test]` | **staging→门②→promote**：rsync staged → 源码测试 → **门② 真实数据闸门** → build wheel → 归档 `wheelhouse/` → 安装公共环境 `/data/hftprop/envs/py311`（hftprop 组 = cken+lgj）→ 装包 smoke |

**门②（staging→gate→promote，findata#27）**：release 模式在「rsync 到 staging `$DST`」之后、
「装入共享公共环境 `$PUBENV`」之前，自动在服务器 py311（conda env vars 已注入 `FINDATA_MYSQL_DSN`、
直连 jydb）对 **staged 源码** 跑 `scripts/release_gate_realdata.py` 真实数据零差对账。**不过即中止，
绝不往下顶替别人在用的公共环境**（staged 源码留在 `$DST` 可排查）。服务器无 jydb 的应急场景用
`--release --skip-realdata-gate` 跳过（风险自负）。
...
release 模式额外确认五件事：
1. **门② 通过**：`==> 门②  服务器真实数据零差对账` 后出 `门① 通过 ✅`（gate 脚本同款输出）——
   不过会在此中止、不进 build/install，公共环境不受影响
2. wheel 构建成功（`cube-0.1.0` + `findata-X.Y.Z` 两个 `py3-none-any`）
3. `pip check` 无报错
4. smoke 三行全出：`findata X.Y.Z` 版本号正确 / `catalog (N, 7)` / `stock_quote(mock)` 形状正常
5. `wheelhouse/` 留痕齐全：wheel、`constraints-vX.Y.Z.txt`、`RELEASES.log` 新行
