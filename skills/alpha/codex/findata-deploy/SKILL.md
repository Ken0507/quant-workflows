...
通过 `~/alpha_projects/.codex/skills/findata-deploy/deploy_to_xinjiang.sh` 把 findata + cube 部署到
...
| 模式 | 命令 | 影响范围 |
|---|---|---|
| **dev**（默认） | `bash ~/alpha_projects/.codex/skills/findata-deploy/deploy_to_xinjiang.sh [--no-test]` | 只更新服务器源码副本 `/data/hftprop/infra/{findata,cube}`（仅 cken 可见，= staging），供自己调试；公共环境与使用方无感知 |
| **release** | `bash ~/alpha_projects/.codex/skills/findata-deploy/deploy_to_xinjiang.sh --release [--no-test]` | **staging→门②→promote**：rsync staged → 源码测试 → **门② 真实数据闸门** → build wheel → 归档 `wheelhouse/` → 安装公共环境 `/data/hftprop/envs/py311`（hftprop 组 = cken+lgj）→ 装包 smoke |
