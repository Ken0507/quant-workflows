...
> 4. **如果当前平台要求用户显式授权子 Agent，则必须在研究开始前获得该授权。** 未获授权前，**不得**用主进程自审、显式 checklist 或其他非独立流程替代 Reviewer SubAgent、Compliance Monitor SubAgent、Logger SubAgent 或独立咨询节点。

## 0. 启动前强制步骤

### 0.0 子 Agent 授权检查

如果当前运行平台对 `Reviewer SubAgent`、`Compliance Monitor SubAgent`、`Logger SubAgent` 或独立咨询节点需要用户显式授权，则必须在 Phase 0 期间先取得该授权，再继续后续流程。

- 未获授权前：停止在 Phase 0，不得进入 Phase 1
- 未获授权前：不得用主进程自审替代独立角色
- 推荐用户授权语句：
  `本次明确授权你使用子 Agent / 多 Agent / 并行 Agent 完成 Reviewer、Compliance Monitor、Logger 以及独立咨询审查，无需再次询问。`
...
### 0.6 研究报告标识头（Identity Header，必填）
...
- **生命周期**：Phase 1 开始时 spawn 一次并保存 reviewer agent id。后续所有审查都发送给同一个 reviewer subagent，**Reviewer 保留完整的历次审查记忆**。
- **默认配置**：`model: gpt-5.4`，`reasoning_effort: xhigh`
- **持久化能力**：
  - 追踪历次提出的建议及回应状态（如"R3 提的 X 在 R7 仍未回应"）
  - 检测跨轮次趋势（如"连续 4 轮无新发现，建议转向"）
  - Phase 终审时基于全程 context 给出比无状态 SubAgent 更深入的判断
- **context 压缩应对**：如果研究超过 15 轮，在发送给 reviewer subagent 的消息中附带未回应建议清单，防止 auto-compaction 丢失关键历史。
...
- **生命周期**：Phase 1 开始时 spawn 一次并保存 compliance agent id。后续所有检查都发送给同一个 compliance subagent，**Compliance Monitor 保留历次检查记忆**。
- **默认配置**：`model: gpt-5.4`，`reasoning_effort: xhigh`
- **持久化能力**：
...
- **生命周期**：Phase 1 开始时 spawn 一次并保存 logger agent id。常规轮次后台并行执行，不阻碍主进程；Phase 边界需要同步等待其完成。
- **默认配置**：`model: gpt-5.4`，`reasoning_effort: xhigh`
- **强制触发节点**（每个都必须触发，不可跳过）：
...
### Codex（结构化咨询伙伴，通过独立 Consultant SubAgent 实现）

通过一个**全新创建的 Consultant SubAgent** 与 Codex 交流。该 consultant subagent 独立于 Reviewer / Compliance / Logger，不复用任何持久化角色。
...
1. 接收用户输入（一句话种子想法）
2. **先写入 §0.6 要求的研究执行者标识块**，再写入 `research_report.md` 的 §0 部分：核心假设、直觉来源、微观行为猜测、与已有因子关系（执行 §0.4）
3. **不需要预定义因子列表**
...
for round in 1..N:
    0. 【前置门控 — 不可跳过】
       a. 如果 round > 1:
          ...
          - 如果标记不存在 → 立即触发 reviewer 持久化 subagent 补执行
            上一轮审查，等待返回后才能继续
       ...
       b. 如果 round ∈ {4, 7, 10, 13, ...}（即每 3 轮 Reviewer 通过后）:
          ...
          - 触发 compliance 持久化 subagent 执行轻量合规检查（见 §4.1）
          - 如有不合规项 → 必须纠正后继续
    ...
    5. 触发 reviewer 持久化 subagent 审查本轮质量 — 每轮必须触发，不可合并或跳过
...
while True:
    1. 准备审查材料
    2. 启动独立 Consultant SubAgent 审查
    3. 处置审查建议
    4. 如有被采纳的建议 → 执行 → 独立思考 → 可能继续研究
    5. 再次认为饱和 → 回到步骤 1
    6. Consultant SubAgent 认为饱和 且 主进程无新方向 → 退出循环，进入终审
```
...
使用同一份审查 brief，启动一个全新的 consultant subagent：

**Codex Consultant SubAgent**：
```
请独立评估：(a) 是否有未被探索的重要方向？(b) 现有机制理解是否有逻辑漏洞？(c) 因子设计空间是否被充分探索？如果你认为有值得追加的探索方向，请给出具体的、可执行的建议（币种/时段/观察重点）。
```

**步骤 3：处置审查建议**

等待 consultant subagent 返回后，对每条建议写处置表（见 §1 Codex 建议跟进纪律），明确采纳/拒绝/推迟及理由。
...
- consultant subagent 认为饱和（无具体可执行建议）**且**主进程也无新方向 → 退出循环
...
- 启动一个 fresh consultant subagent，将候选因子列表 + Phase 1 机制摘要提交给它，提问：
...
- 任何一轮 Reviewer 不通过时，由主进程立即触发 compliance 持久化 subagent
