...
1. **重新完整阅读** `/home/cken/.claude/skills/crypto-deep-factor-research/SKILL.md`
...
### OB/Trade 调查记录格式（按 `target_and_workflow.md` §1.7 六要素核对）
对每轮调查检查：
- [ ] 是否记录了币种/日期/时段？
- [ ] 是否记录了调查目的（来源于什么前序发现或假设）？
- [ ] 是否**详细描述**了观察到的现象（不是一句话概括）？是否有独立的 `#### 观察到的现象（详细描述）` 小节？
- [ ] 是否包含具体数据点（AggTrans 特征、L2 book 快照、事件时间线、seq / ts_ms、flow / sweep / depth 量化指标）？
- [ ] 是否引用了 crypto-research MCP 工具（`get_events` / `get_book` / `analyze_sweep` / `flow_analysis` / `liquidity_profile` / `inspect_bar` / `forward_returns` 等）的具体输出？
- [ ] 是否有假设与结论对比分析？（一致/不一致/意外发现的详细分析）
- [ ] 是否有因子设计启示？
- [ ] 是否有**效应时间特征**小节？（horizon 上的响应形态、regime 依赖、跨币种时滞等，定性即可）
- [ ] 是否有新问题/新发现（驱动下一轮调查）？
...
- [ ] 抽查最近 2 轮的 report 记录是否包含 §1.7 完整六要素（目的/现象/假设对比/因子启示/效应时间特征/新问题）？
