---
name: sync-skills
description: "扫描 quant-workflows/skills/ 全部 skill 目录，对照实际软链接做同步。适用于：新增、迁移、重命名或删除 skill 后，或怀疑 Claude/Codex 软链接未对齐时。缺失的软链接自动创建；孤立的软链接（指向不存在的源）只报告不删除。"
---
...
无参数。当 `quant-workflows/skills/` 下新增、迁移、重命名或删除 skill，或怀疑 `.claude/.codex` 软链接状态未对齐时，应主动调用。

## 执行流程

**全部工作通过独立 Codex subagent 完成。**
