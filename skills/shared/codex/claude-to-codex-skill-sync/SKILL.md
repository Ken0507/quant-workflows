...
description: "将单个已存在的 Codex skill 与其 Claude 源 skill 的后续更新做增量同步。适用于：quant-workflows 中某个 Claude skill 更新后，需要以该 Codex skill 上次更新 commit 为基线，分析之后的 Claude commits，并只同步发生变化的部分；平台绑定边界与 claude-to-codex-skill-migration 保持一致。"
---
...
把一个**已存在**的 Codex skill，与其对应 Claude skill 在后续 commit 中发生的更新做**增量同步**，同时保持 Codex 平台适配语义不被冲掉。
