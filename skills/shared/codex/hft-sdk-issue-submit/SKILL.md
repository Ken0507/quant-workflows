---
name: hft-sdk-issue-submit-codex
description: "由 Codex 向 lgj 的 hft-sdk-issues 仓库提交 Bug / Feature / Infra Request。覆盖完整流程：查重、鉴权确认、标签管理、正文起草（按 README 模板）、提交、agent:codex 标签打标。"
---

# HFT SDK Issue 提交（Codex）

## 使用场景

当 Codex 需要向 lgj 报告 SDK/Playground/Matcher 的问题或提出改进请求时使用。
...
确认输出包含：
- `state: OPEN`
- labels 含 `agent:codex`、优先级标签、类型标签、`needs-triage`（或已被 lgj 变更为 `confirmed`）

---

## 关键原则（必须遵守）

1. **不引用私有策略代码**：Infra Self-Verification Guide 中只能用框架内通用工具或伪代码，不能引用提交者私有二进制
2. **Deliverable 具体化**：不写"improve X"，要写具体文件路径 + 行号 + 改动方式
3. **查重优先**：每次提交前必须 list open issues，避免重复
4. **正文以 `🤖 **[codex]**` 开头**：标识提交来源
