...
设计原则：
- **只杀 agent 自己启动的进程**：通过爬 `/proc/<pid>/stat` 的 PPID 链，定位到 Claude 主进程，然后 BFS 遍历它的所有后代作为「受管集合」。不在这个集合里的进程（用户自己开的 tmux、其他 session 的任务）一概不动。
- **永远不杀 root（Claude 主进程）本身**，也不杀守护自己。
...
```
skills/shared/claude/memory-guard/memory_guard.sh
```

### 启动（默认 400 GB 阈值）

```bash
bash skills/shared/claude/memory-guard/memory_guard.sh start
```
...
默认脚本会从当前 shell 往上爬，找到最近的 `comm` 或 `cmdline` 里含 `claude` 的祖先作为 root。若自动识别失败或你想手动绑定（例如监控某个特定 worker PID 的子树），用 `--root-pid`：

```bash
bash skills/shared/claude/memory-guard/memory_guard.sh start --root-pid 6789
```
...
```bash
bash skills/shared/claude/memory-guard/memory_guard.sh status
```
...
```bash
bash skills/shared/claude/memory-guard/memory_guard.sh check
```
...
```bash
bash skills/shared/claude/memory-guard/memory_guard.sh stop
```
...
6. **root 自动识别不保证正确**：爬到第一个 comm 含 `claude` 的祖先就停。如果你在 tmux 里跑且祖先链很长，建议显式 `--root-pid`。先用 `check` 确认 root 找对了。
...
# 紧凑点的阈值
bash skills/shared/claude/memory-guard/memory_guard.sh start --threshold 200 --interval 3
