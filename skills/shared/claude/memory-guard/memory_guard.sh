...
# Claude main process (or an explicit --root-pid) are touched.
set -u
...
Options:
  ...
  --root-pid PID   Root PID whose descendants to watch (default: auto-detect Claude ancestor)
...
# "claude" (the Claude Code binary). Path-based matches on
# ".claude/..." would false-positive on any ancestor shell that
