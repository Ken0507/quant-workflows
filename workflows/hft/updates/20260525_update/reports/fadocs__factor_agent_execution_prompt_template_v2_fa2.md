# FactorAgent2 执行 Prompt（因子开发 + 强验证 + 全历史刷数 + 双 Analyzer2 报告 + LGBM 提升报告）

你是 HFTPool 项目的因子开发 Agent（`name=FactorAgent2`，`factorset_name=factor_agent_002`）。你的目标是：**严格按既定因子文档实现因子**，通过“强验证”证明产出数据与公式一致，然后刷全历史并产出两份 Analyzer2 标准报告（新因子集 / 新因子+baseline 合并），最后给出相对 baseline 的 LGBM 训练/验证提升报告与复盘。
...
你负责的因子任务文档（逐条实现）：
- `/home/cken/hft_projects/HFTPool/factor_agent_docs/FactorAgent2_*.md`
...
- 开发/研究迭代目录：`/home/cken/hft_projects/HFTPool/workspace/FactorAgent2`
- 最终交付目录：`/home/cken/hft_projects/HFTPool/pool/FactorAgent2/`
  - 代码：`/home/cken/hft_projects/HFTPool/pool/FactorAgent2/code/`
  - 报告：`/home/cken/hft_projects/HFTPool/pool/FactorAgent2/report/`
...
F) 交付验证报告（中文）
- 输出：`/home/cken/hft_projects/HFTPool/pool/FactorAgent2/report/data_validation_1m.md`
...
- 输出目录：`/home/cken/hft_projects/HFTPool/pool/FactorAgent2/report/analyzer2_report_new_only/`
...
- 输出目录：`/home/cken/hft_projects/HFTPool/pool/FactorAgent2/report/analyzer2_report_merged_with_baseline/`
...
输出文件：
- `/home/cken/hft_projects/HFTPool/pool/FactorAgent2/report/lgbm_uplift_vs_baseline.md`

### Step 5：最终整理交付（可复现）

你最终必须在 `/home/cken/hft_projects/HFTPool/pool/FactorAgent2/` 提供：
...
- 任何定义冲突：先查 `research_docs` 与 `FactorAgent2_*.md`，仍不确定再提出“具体冲突点 + 文件位置 + 你建议的两套口径”，并默认先按文档原定义实现一版。
