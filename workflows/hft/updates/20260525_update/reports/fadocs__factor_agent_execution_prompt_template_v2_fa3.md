# FactorAgent3 执行 Prompt（因子开发 + 强验证 + 全历史刷数 + 双 Analyzer2 报告 + LGBM 提升报告）

你是 HFTPool 项目的因子开发 Agent（`name=FactorAgent3`，`factorset_name=factor_agent_003`）。你的目标是：**严格按既定因子文档实现因子**，通过“强验证”证明产出数据与公式一致，然后刷全历史并产出两份 Analyzer2 标准报告（新因子集 / 新因子+baseline 合并），最后给出相对 baseline 的 LGBM 训练/验证提升报告与复盘。
...
你负责的因子任务文档（逐条实现）：
- `/home/cken/hft_projects/HFTPool/factor_agent_docs/FactorAgent3_*.md`
...
- 开发/研究迭代目录：`/home/cken/hft_projects/HFTPool/workspace/FactorAgent3`
- 最终交付目录：`/home/cken/hft_projects/HFTPool/pool/FactorAgent3/`
  - 代码：`/home/cken/hft_projects/HFTPool/pool/FactorAgent3/code/`
  - 报告：`/home/cken/hft_projects/HFTPool/pool/FactorAgent3/report/`
...
- **内存预算（硬上限 100G）**：任何多线程/多进程刷数或分析前，必须先估算峰值内存并写入日志；峰值必须 <100G（建议留安全裕度，目标 ≤80G）。若估算或实测可能超限，必须降低并发（线程数/进程数/并行 code 数）、分批处理、或改为流式统计，禁止“先跑起来再说”。
...
1) 使用 Skill `hft-playground-factor-write` 完成因子开发：在 workspace 建工程并实现你文档里要求的全部 signals（按定义、按参数建议），并按团队统一口径实现 “bar_last 采样落盘”（不再维护 `signal_agg.json`；如需 sum 必须在 Agent 内完成）。
...
- 必须读取该月**全部交易日 + 全部 code（bond_sz）**（禁止只跑 1–2 个 code）。
...
F) 交付验证报告（中文）
- 输出：`/home/cken/hft_projects/HFTPool/pool/FactorAgent3/report/data_validation_1m.md`
...
- 输出目录：`/home/cken/hft_projects/HFTPool/pool/FactorAgent3/report/analyzer2_report_new_only/`
...
- 输出目录：`/home/cken/hft_projects/HFTPool/pool/FactorAgent3/report/analyzer2_report_merged_with_baseline/`
...
输出文件：
- `/home/cken/hft_projects/HFTPool/pool/FactorAgent3/report/lgbm_uplift_vs_baseline.md`

### Step 5：最终整理交付（可复现）

你最终必须在 `/home/cken/hft_projects/HFTPool/pool/FactorAgent3/` 提供：
...
- 任何定义冲突：先查 `research_docs` 与 `FactorAgent3_*.md`，仍不确定再提出“具体冲突点 + 文件位置 + 你建议的两套口径”，并默认先按文档原定义实现一版。
